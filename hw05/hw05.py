...
def replace_leaf(t, old, new):
    ...
    "*** YOUR CODE HERE ***"
    if(is_leaf(t)):
        return tree(label(t)==old and new or label(t),[])
    return tree(label(t),[replace_leaf(e,old,new) for e in branches(t)])
...
def size(w):
    ...
    "*** YOUR CODE HERE ***"
    assert is_weight(w), "Must be a weight"
...
def balanced(m):
    ...
    for e in sides(m):
        if(is_mobile(end(e))):
            children_balanced = children_balanced and balanced(end(e))

    b = [total_weight(end(e))*length(e) for e in sides(m)]
    return b[0]-b[1]==0 and children_balanced
...
class FreeChecking(Account):
    ...
    def withdraw(self, amount):
        ...
        return super(FreeChecking,self).withdraw(amount)
...
def make_withdraw(balance, password):
    ...
    "*** YOUR CODE HERE ***"
    attempts = []
    def another(amount, input_password):
        nonlocal balance
        if(len(attempts)==3):
            return "Your account is locked. Attempts: " + repr(attempts)
